...
export default function GuidelinesPage() {
  const sections = [
    ...
    },
    {
      ...
      bg: "bg-flame-500/10",
      rules: [
        "Participant domains must strictly be @srec.ac.in",
    ...
    <AdminLayout>
      <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-flame-500/10 text-flame-600 rounded-full text-xs font-bold uppercase tracking-widest border border-flame-500/20">
            <BookOpen size={14} />
            Operating Procedures
          </div>
          <h1 className="text-5xl font-bold text-ash-900 dark:text-white font-heading tracking-tight">
            The CoIN <span className="text-gradient">Protocol</span>
          </h1>
          <p className="text-ash-500 dark:text-ash-400 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {sections.map((section, idx) => (
            <div key={idx} className="bg-white dark:bg-ash-900 border border-ash-200 dark:border-ash-800 p-8 rounded-[40px] shadow-sm hover:shadow-xl hover:shadow-black/5 transition-all">
              ...
              <h2 className="text-2xl font-bold text-ash-900 dark:text-white mb-6 font-heading">{section.title}</h2>
              <ul className="space-y-4">
                {section.rules.map((rule, ridx) => (
                  <li key={ridx} className="flex items-start gap-4">
                    <CheckCircle2 className="text-ash-300 dark:text-ash-700 mt-1 flex-shrink-0" size={18} />
                    <span className="text-ash-600 dark:text-ash-400 text-sm leading-relaxed">{rule}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="bg-ash-900 dark:bg-ash-950 rounded-[40px] p-10 text-white overflow-hidden relative">
          <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
            <div className="w-20 h-20 bg-white/10 rounded-3xl flex items-center justify-center text-flame-400 flex-shrink-0">
              <MessageSquare size={40} />
            </div>
            <div className="space-y-4 text-center md:text-left">
              <h3 className="text-2xl font-bold font-heading">Protocol Deviation?</h3>
              <p className="text-ash-400 max-w-xl">
        ...
        {/* Footer Warning */}
        <div className="flex items-center justify-center gap-3 text-ash-400 text-xs font-bold uppercase tracking-[0.3em] py-10 opacity-50">
          <AlertCircle size={14} />
