...
const Button = ({ variant = "default", size = "sm", className = "", children, ...props }: ButtonProps) => {
    ...
    const variants = {
        default: "bg-white text-black hover:bg-ash-100",
...
/**
 * Ethereal Beams Hero
 * Refactored to be a reusable Hero section for CoIN.
 */
export default function EtherealBeamsHero({
    ...
    return (
        <div className="relative min-h-[90vh] md:min-h-screen w-full overflow-hidden bg-black flex items-center pt-20 md:pt-0">
            {/* Beams Background */}
                ...
                <div className="mx-auto max-w-7xl px-6 lg:px-8">
                    <div className="mx-auto max-w-4xl text-center">
                        ...
                        <div className="mb-8 inline-flex items-center rounded-full bg-white/5 backdrop-blur-xl border border-white/10 px-4 py-2 text-sm text-white/90">
                            ...
                            <span className="opacity-70 mr-2 uppercase tracking-widest text-[10px]">{badgeLabel}</span>
                            <span className="font-bold">{badgeText}</span>
                        </div>

                        {/* Main Heading */}
                        <h1 className="mb-6 text-4xl font-bold tracking-tight text-white sm:text-6xl lg:text-7xl font-display uppercase leading-[0.95]">
                            {title.split(' ').map((word, i) => (
                                <span key={i} className={i === 2 ? "bg-gradient-to-r from-white via-ash-200 to-ash-400 bg-clip-text text-transparent" : ""}>
                                    {word}{' '}
                                </span>
                            ))}
                        </h1>
                        ...
                        {/* Subtitle */}
                        <p className="mb-10 text-lg leading-8 text-white/70 sm:text-xl lg:text-2xl max-w-3xl mx-auto font-light">
                            {description}
                        </p>
                            ...
                            {ctaButtons.map((btn, i) => (
                                <Link key={i} href={btn.href || '#'} className="w-full sm:w-auto">
                                    <Button
                                        ...
                                        size="lg"
                                        className={btn.primary ? "shadow-2xl shadow-white/20 font-bold w-full" : "font-semibold bg-transparent w-full"}
                                    >
                                        {btn.text}
                            ...
                            <div className="flex flex-wrap justify-center gap-x-8 gap-y-4 max-w-2xl mx-auto">
                                {microDetails.map((detail, i) => (
                                    <div key={i} className="flex items-center gap-2">
                                        ...
                                        <span className="text-white/50 text-[10px] uppercase tracking-[0.2em] font-medium">{detail}</span>
                                    </div>
                                ))}
                            </div>
                        )}
